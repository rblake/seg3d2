...
set(HAVE_LONG_LONG "8")
set(SIZEOF_VOID_P "${CMAKE_SIZEOF_VOID_P}")
set(SIZEOF_SIZE_T "${CMAKE_SIZEOF_VOID_P}")
set(HAVE_SIZEOF_SIZE_T "${CMAKE_SIZEOF_VOID_P}")
...
IF(NOT APPLE) #libintl.h isn't available on Mac OS X 
...
add_cond(CFG_HEADERS HAVE_SYS_LOADAVG_H sys/loadavg.h)
...
check_symbol_exists(acosh        "${CFG_HEADERS}" HAVE_ACOSH)
check_symbol_exists(alarm        "${CFG_HEADERS}" HAVE_ALARM)
check_symbol_exists(altzone      "${CFG_HEADERS}" HAVE_ALTZONE)
check_symbol_exists(asinh        "${CFG_HEADERS}" HAVE_ASINH)
check_symbol_exists(atanh        "${CFG_HEADERS}" HAVE_ATANH)
...
check_symbol_exists(copysign     "${CFG_HEADERS}" HAVE_COPYSIGN)
...
check_symbol_exists(erf          "${CFG_HEADERS}" HAVE_ERF)
check_symbol_exists(erfc         "${CFG_HEADERS}" HAVE_ERFC)
check_symbol_exists(execv        "${CFG_HEADERS}" HAVE_EXECV)
check_symbol_exists(expm1        "${CFG_HEADERS}" HAVE_EXPM1)
...
check_symbol_exists(finite       "${CFG_HEADERS}" HAVE_FINITE)
...
check_symbol_exists(gamma        "${CFG_HEADERS}" HAVE_GAMMA)
...
check_symbol_exists(lgamma       "${CFG_HEADERS}" HAVE_LGAMMA)
check_symbol_exists(link         "${CFG_HEADERS}" HAVE_LINK)
check_symbol_exists(log1p        "${CFG_HEADERS}" HAVE_LOG1P)
...
check_symbol_exists(round        "${CFG_HEADERS}" HAVE_ROUND)
...
check_symbol_exists(tgamma       "${CFG_HEADERS}" HAVE_TGAMMA)
...
#Disable dynamic loading since we are building static python
#if(HAVE_DLOPEN) # OR .... )
...
set(HAVE_GCC_ASM_FOR_X87 1)
set(WANT_SIGFPE_HANDLER 1)
