...
#include <Core/IRTools_tmp/the_dynamic_array.hxx>

#include <Core/ITKCommon/FFTWrapper.h>
...
#else
#define DEBUG_PDF_PFX the_text_t("")
#endif
...
find_maxima_cm(std::list<local_max_t> & max_list,
	       ...
	       const the_text_t & prefix = DEBUG_PDF_PFX,
	       const the_text_t & suffix = the_text_t(".png"));
		      ...
		      const mask_t * mask_b = NULL)
{
  ...
  for (unsigned int i = 0; i < 4; i++)
  {
    ...
    int old_precision = log.precision();
    log.precision(2);
    log << i << ": " << setw(3) << std::fixed << area_ratio * 100.0
	<< "% of overlap, ";
	       ...
	       const unsigned int max_peaks)
{
  ...
  double m = match_one_pair<TImage, TMask>(log,
					   images_were_resampled,
					   use_std_mask,
					   fi,
					   mi,
	       ...
	       translate_transform_t::Pointer & ti)
{
  ...
  return match_one_pair<TImage, TMask>(log,
				       images_were_resampled,
				       use_std_mask,
				       fi,
				       mi,
