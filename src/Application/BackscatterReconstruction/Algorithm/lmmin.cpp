...
{
  return p[0] * exp((-(x-p[1])*(x-p[1]))/(2.0*p[2]*p[2])) + p[3];
}
...
double lm_gaussian_fit_2d(double x, double y, double *p)
{
  ...
  return p[5] + p[0] * exp(-((dx*dx)/(2.0*p[2]*p[2]) +
                             (dy*dy)/(2.0*p[4]*p[4])));
}
...
double lm_gaussian_fit_2dg(double x, double y, double *p)
{
  ...
  return p[5] + dx*p[7] + dy*p[8] +
    //p[9]*dx*dx + p[10]*dx*dy + 2.6*p[11]*dy*dy +
    p[0] * exp(-((dx*dx)/(2.0*p[2]*p[2]) +
                 (dx*dy)*p[6] +
                 (dy*dy)/(2.0*p[4]*p[4])));
}
 ...
 *
 *	data is a read-only pointer to lm_data_1d
 *
 ...
 */
{
  int i;
  lm_data_1d *mydata;
  mydata = (lm_data_1d *) data;

  for (i = 0; i < m_dat; i++)
    fvec[i] = mydata->fvec[i] - mydata->f(mydata->xvec[i], par);
...
void lm_evaluation_2d(double *par, int m_dat, double *fvec, void *data, int *info)
{
  int i;
  lm_data_2d *mydata;
  mydata = (lm_data_2d *) data;

  for (i = 0; i < m_dat; i++)
    fvec[i] = mydata->fvec[i] - mydata->f(mydata->xvec[i], mydata->yvec[i], par);
 ...
 */
{
  ...
  if (iflag == 2) {
    std::cerr << "trying step in gradient direction\n"<<std::endl;
  } else if (iflag == 1) {
    std::cerr << "determining gradient (iteration " << iter << ")\n"<<std::endl;
  } else if (iflag == 0) {
    std::cerr << "starting minimization\n"<<std::endl;
  } else if (iflag == -1) {
    std::cerr << "terminated after " << nfev << " evaluations\n"<<std::endl;
  }

  std::cerr << "  par: "<<std::endl;
  for (i = 0; i < n_par; ++i)
    std::cerr << " " << par[i] << " => norm: " << lm_enorm(m_dat, fvec)<<std::endl;

  if (iflag == -1) {
    std::cerr << "  fitting data as follows:\n"<<std::endl;
    for (i = 0; i < m_dat; ++i) {
      t = (mydata->tvec)[i];
      y = (mydata->yvec)[i];
      f = mydata->f(t, par);
      std::cerr << "    t[" << i << "]=" << t << " y=" << y << " fit=" << f << " residue=" << y-f<<std::endl;
    }
  }
#endif
}
...
void lm_print_2d(int n_par, double *par, int m_dat, double *fvec, void *data, int iflag, int iter, int nfev)
{
...
#if 0
  int i;
  lm_data_2d *mydata = (lm_data_2d *) data;
  ...
  if (iflag == -1) {
    std::cerr << "  fitting data as follows:\n"<<std::endl;
    for (i = 0; i < m_dat; ++i) {
      double tx = (mydata->xvec)[i];
      double ty = (mydata->yvec)[i];
      double tf = (mydata->fvec)[i];
      double f = mydata->f(tx, ty, par);
      std::cerr << "    x[" << i << "]=" << tx <<"    y[" << i << "]=" << ty <<"    f[" << i << "]=" << tf <<
        " fit=" << f << " residue=" << tf-f<<std::endl;
    }
  }
#endif
}
